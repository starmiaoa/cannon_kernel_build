...
OUTPUT_ZIP=$TOOLCHAIN/Cannon-Stock-OriginOS4-OfficialSource.zip
...
cd "$AK_DIR"
rm -f Image.gz Cannon-Stock-OriginOS4-OfficialSource.zip
...
ls -lh "$OUTPUT_ZIP" "$OUT_DIR/arch/arm64/boot/Image.gz" \
  "$TOOLCHAIN/originos4-official-result.config" "$TOOLCHAIN/originos4-official-config.diff"
